...
class NejmensiKurzyPresenter extends BasePresenter
{
    ...
    public function selectFormSucceeded(UI\Form $form, $values)
    {
        ...
        $this->nej_kurzy->zobrazit($form, $values, $this->template->nabidky, $this->template->kurzy_pozice, $this->template->nabidky_kurzy, $this->template->nabidky_moznosti, $this->template->i, $this->template->kurzy_pole, $this->template->nabidky_moznosti_id, $this->template->nabidky_moznosti_id_pole, $this->template->id_tiketu, $this->template->sazkovka, "ASC");
    }
}